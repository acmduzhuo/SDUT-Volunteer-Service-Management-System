...
<table class="layui-table" lay-data="{url:'guanli_list', page:true, id:'guanlitable_id',toolbar:'#toolbar'}" lay-filter="guanlitable">
  <thead>
    <tr>
...
<form  hidden id="h_div" class="layui-form "  lay-filter="guanlisave">
	...
	<form  hidden id="update_h_div" class="layui-form "  lay-filter="guanliupdate">
...
var form = layui.form;
table.on('toolbar(guanlitable)', function(obj) {
	//获取选中数据
	var checkStatus = table.checkStatus('guanlitable_id');
	...
	case 'del':
		...
		if(data.length==0){
			...
			msg="确定要删除"+data[0].User_realname+"管理员的信息吗？";
		}else{
			msg="确定要删除"+data[0].User_realname+"等，"+data.length+"条管理员的信息吗？";
		}
		...
		layer.confirm(msg,{
			...
			for(var i=0;i<data.length;i++){
				guanliids+=data[i].User_id+",";
			}
			$.post("guanli_del_more",{"guanliids":guanliids}, function(d){
				//删除后关闭对话框
				layer.closeAll();
				layer.msg('管理员信息删除成功',{icon:6,time:2000});
				//表格重新加载
	...
	case 'insert':
		...
		layer.open({
					...
					btn : [ '确定', '取消' ],
					btn1 : function() {
						$.post("guanli_save",form.val("guanlisave"), function() {
							layer.closeAll();
							layer.alert('默认密码:123456',{title:'新增管理员信息'});
							table.reload('guanlitable_id');
						});
					},
					btn2 : function() {
...
table.on('tool(guanlitable)', function(obj) {
	var data = obj.data;
	//console.log(data);
	switch (obj.event) {
	case 'update':
		//$("update_h_div [name=user_sex]").val(data.user_sex);
		form.val("guanliupdate",{
		...
		layer.open({
					...
					btn : [ '确定', '取消' ],
					btn1 : function() {
						$.post("guanli_update",form.val("guanliupdate"), function() {
							layer.closeAll();
							layer.msg(data.User_realname+'管理员信息修改成功！',{icon:6,time:2000});
							table.reload('guanlitable_id');
						});
					},
					btn2 : function() {
	...
	case 'del':
		...
		layer.confirm("确定要删除"+data.User_realname+"管理员的信息吗？",{
			btn:['确定','取消']
			...
			//点击确定之后执行
			//将要删除的数据发送到后台
			$.post("guanli_del",data, function(d){
				//删除后关闭对话框
				layer.closeAll();
				//表格重新加载
				table.reload('guanlitable_id');
